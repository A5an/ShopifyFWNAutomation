 ...
 */
export async function extractPdfTables(pdfPath: string, method: string = 'invoice'): Promise<PythonExtractionResult> {
  return new Promise((resolve, reject) => {
    ...
    pythonProcess.on('close', (code) => {
      ...
      try {
        ...
        if (!lastJsonLine) {
          throw new Error(`No valid JSON found in output. stdout: ${stdout}, stderr: ${stderr}`);
        }
        
        const result: PythonExtractionResult = JSON.parse(lastJsonLine);
        console.log(`✅ Successfully extracted ${result.total_found || (result.tables ? result.tables.length : 0)} tables`);
        resolve(result);
 ...
 */
export async function extractPdfTablesEnhanced(pdfPath: string): Promise<PythonExtractionResult> {
  try {
    ...
    if (invoiceResult.error) {
      console.log('🔄 Trying all methods extraction due to invoice method error...');
      ...
      if (allTables.length > 0) {
        ...
        const invoiceTables = allTables.filter(table => {
          ...
          return table.shape[1] >= 3 &&  // At least 3 columns
        ...
        if (invoiceTables.length > 0) {
          console.log(`🎯 Found ${invoiceTables.length} potential invoice tables`);
          return {
